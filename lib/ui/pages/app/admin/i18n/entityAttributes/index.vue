<template>
  <GenericResourceOverview :columns="columns" resource-name="i18nEntityAttribute" ref="overview">
    <template #menuItems>
    ...
    <template #filter="{filterContext}">
      <div class="filter-row">
        <div class="filter-field-wrapper">
          <text-field-filter filter-key="entityClass" :caption="i18n.translate('Entity Class').build()" :table-filter-context="filterContext" />
        </div>
        <div class="filter-field-wrapper">
          <text-field-filter filter-key="text" :caption="i18n.translate('Text').build()" :table-filter-context="filterContext" />
        </div>
        <div class="filter-field-wrapper">
          <text-field-filter filter-key="attribute" :caption="i18n.translate('Attribute').build()" :table-filter-context="filterContext" />
        </div>
      </div>
      <div class="filter-row">
        <div class="filter-field-wrapper">
  ...
  <harmony-modal v-model="isImportModalVisible" :is-minimal="true" :caption="i18n.translate('Import').build()">
    ...
    <div v-else>
      <p><b>{{i18n.translate('Imported Elements').build()}}: </b> {{fileUploadResponse.data["importedElements"]}}</p>
  ...
  <harmony-modal v-model="isExportModalVisible" :is-minimal="true" :caption="i18n.translate('Export').build()">
    <div class="modal-row">
...
const i18n = I18N.of("entityAttributes-index")
...
const columns:HarmonyTableColumn[] = [
  {
    id: "entityClass",
    caption: i18n.translate("Entity Class").build()
  },
  {
    id: "attributeName",
    caption: i18n.translate("Attribute Name").build()
  },
  {
    id: "isCoreEntry",
...
const onExportEntries = async () => {
  ...
  await fileUtils.downloadFileFromApi(ECoreBackendPath.API_I18NENTITYATTRIBUTE_EXPORT, queryParams)
}

const coreApi = useCoreApi()
onMounted(async () => {
  translationStatistics.value = (await coreApi.api().getI18nEntityAttributeApi().getStatistics()).data
})
...
const uploadImportFile = async () => {
  ...
  const coreApi = useCoreApi()
  const response = await coreApi.api().getI18nEntityAttributeApi().importKeyEntryData(fileToUpload)
