<template>
  ...
  <div v-if="!isRegistrationFromScratchAllowed">
      ...
      <div class="content">
        <form id="register" @submit.prevent="onSubmitClick" style="width: 100%; max-width: 40em;">
          <harmony-panel caption="Register" class="panel">
            ...
            <form-text-field field-path="password"  :caption="i18n.translate('Password').build()" autocomplete-name="new-password" :is-password="true"/>
            <form-text-field field-path="passwordAgain"  :caption="i18n.translate('Password again').build()" autocomplete-name="new-password" :is-password="true"/>
            <div class="btn-row">
...
const i18n = I18N.of("register")

const coreApi = useCoreApi()
const isRegistrationFromScratchAllowed = (await coreApi.api().getUserRegistrationApi().getConfiguration()).data.isRegistrationFromScratchAllowed

const currentState = ref("WAITING_FOR_USER_INPUT")

const viewModelContext = useViewModelContext("UserRegistrationVM")
...
const onSubmitClick = async (e:any) => {
  ...
  if(!isRegistrationFromScratchAllowed)
